...
public class LLMService : ILLMService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var llavaWeights = visionModel?.MMProjectName is not null
            ? await LLavaWeights.LoadFromFileAsync(
                Path.Combine(modelsPath, visionModel.MMProjectName),
                cancellationToken)
            : null;
    ...
    private static LocalModel GetLocalModel(string modelId)
    {
        ...
        return model is not LocalModel localModel
    ...
    private async Task<ChatResult> CreateChatResult(
        Chat chat,
