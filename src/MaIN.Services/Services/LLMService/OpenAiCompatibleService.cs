    ...
    : ILLMService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            model = chat.Model,
            messages = await BuildMessagesArray(conversation, chat),
            stream = true
        };
    ...
    private async Task<object[]> BuildMessagesArray(List<ChatMessage> conversation, Chat chat)
    {
        ...
        foreach (var msg in conversation)
        {
            ...
            // Check if this is an original message with potential image
            if (msg.OriginalMessage != null)
            {
                content = await BuildMessageContent(msg.OriginalMessage);
            }
            else
            {
                content = msg.Content;
    ...
    private async Task<object> BuildMessageContent(Message message)
    {
