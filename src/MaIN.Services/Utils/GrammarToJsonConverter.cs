...
public class GrammarToJsonConverter
{
    ...
    public GrammarToJsonConverter(int maxDepth = 10)
    {
    ...
    /// <summary>
    /// Parse chosen grammar and convert to JSON schema example
    /// </summary>
    ...
    public string ConvertToJson(Grammar grammar)
    {
        var result = grammar.Format switch
        {
            GrammarFormat.GBNF => ConvertFromGBNF(grammar.Value),
            GrammarFormat.JSONSchema => grammar.Value,
            _ => throw new NotSupportedException($"Grammar format '{grammar.Format}' is not supported")
        };
    ...
    public string ConvertFromGBNF(string gbnfText, string startRule = "root")
    {
