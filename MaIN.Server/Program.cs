...
{
    var result = await llmService.AskMemory(request.Chat, request.Jsons, request.FilePath, request.Memory);
    return result != null 
        ? Results.Ok(result) 
...
record AskMemoryRequest(Chat Chat, List<string>? Jsons = null, string? FilePath = null, List<string>? Memory = null);